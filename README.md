...
卡片按钮：打断、离开（仅 B）、回答 `ask`/`select` 选项。正文里出现 `1. 2.` 选项时也可点选。A 空闲时还有「模型」「思考」。

每条新 prompt 发一张新卡片，避免多轮后要往上翻；新卡片只显示本轮，不会先闪出上一轮输出。本轮结束（或出现选项）会走飞书原生通知：有加急权限就加急该卡片，否则发一条短文本。每条飞书对话记住自己的 omp 会话文件（`~/.omp-feishu/chats.json`），下次还是接着聊。
